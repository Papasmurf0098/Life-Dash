    ...
    <div className="feature-page home-page">
      ...
      <section className="home-grid">
        <button className="home-card home-card--priority" type="button" onClick={() => onNavigate('bulletin')}>
          <div className="home-card__heading"><span className="card-icon">✦</span><div><p className="kicker">What matters now</p><h2>{topPriority?.title ?? 'Your headspace is clear'}</h2></div></div>
        ...
        <button className="home-card home-card--earnings" type="button" onClick={() => onNavigate('tips')}>
          <div className="home-card__heading"><span className="card-icon">↗</span><div><p className="kicker">Projected shift earnings</p><h2>{dollars.format(snapshot.week.projectedEarnings)}</h2></div></div>
        ...
        <button className="home-card home-card--budget" type="button" onClick={() => onNavigate('budget')}>
          <div className="home-card__heading"><span className="card-icon">$</span><div><p className="kicker">Take-home budget</p><h2>{formatCurrency(snapshot.budget.totalIncomeCents)}</h2></div></div>
        ...
        <article className="home-card home-card--forecast">
          <div className="home-card__heading"><span className="card-icon">◎</span><div><p className="kicker">Month pace</p><h2>{dollars.format(snapshot.monthProjection)}</h2></div></div>
      ...
      <section className="glass-panel quick-launch">
        <div><p className="kicker">Quick launch</p><h2>Move without hunting</h2></div>
        <div className="quick-launch__actions"><button type="button" onClick={() => onNavigate('bulletin')}><span>＋</span>Add thought</button><button type="button" onClick={() => onNavigate('tips')}><span>↗</span>Log shift</button><button type="button" onClick={() => onNavigate('budget')}><span>$</span>Record take-home pay</button></div>
      </section>
    </div>
  )
}
