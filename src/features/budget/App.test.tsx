...
describe('App', () => {
  ...
  it('updates totals when adding an expense', async () => {
    ...
    const startingAmount = screen.getByLabelText(/expected monthly take-home/i)
  ...
  it('tracks only recorded take-home pay entries', async () => {
    const user = userEvent.setup()
    render(<App />)

    await user.type(screen.getAllByLabelText(/take-home amount/i)[0], '1200')
    await user.type(screen.getAllByLabelText(/take-home note/i)[0], 'Paycheck')
    await user.click(screen.getAllByRole('button', { name: /add take-home pay/i })[0])
    ...
    expect(
      screen
        .getAllByRole('status')
        .some((node) => node.textContent?.match(/recorded total updated/i)),
    ).toBe(true)
  })
